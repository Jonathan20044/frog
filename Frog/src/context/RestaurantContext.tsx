...
interface RestaurantContextType {
  ...
  updateItemQuantity: (itemId: number, quantity: number, notes?: string, selectedOptions?: Record<string, string>) => void;
  confirmOrder: () => void;
...
export const menuItems: MenuItem[] = [
  ...
  { 
    ...
    customizations: [
      { id: 'base', name: 'Base de la sopa', options: ['Con leche', 'Con agua', 'Con crema'], required: true },
      { id: 'size', name: 'Tamaño', options: ['Pequeña', 'Mediana', 'Grande'], required: true }
    ]
  },
  { 
    id: 3, 
  ...
  { 
    ...
    customizations: [
      { id: 'coccion', name: 'Término de cocción', options: ['Poco cocido', 'Término medio', 'Tres cuartos', 'Bien cocido'], required: true },
      { id: 'acompanamiento', name: 'Acompañamiento', options: ['Papas fritas', 'Puré', 'Arroz', 'Ensalada'], required: true }
    ]
  },
  { 
    id: 5, 
  ...
  { 
    ...
    customizations: [
      { id: 'coccion', name: 'Término de cocción', options: ['Jugoso', 'Término medio', 'Bien cocido'], required: true },
      { id: 'salsa', name: 'Salsa', options: ['Limón y mantequilla', 'Teriyaki', 'Al natural'], required: false }
    ]
  },
  { 
    id: 7, 
...
export function RestaurantProvider({ children }: { children: ReactNode }) {
  ...
  const updateItemQuantity = (itemId: number, quantity: number, notes?: string, selectedOptions?: Record<string, string>) => {
    if (quantity <= 0) {
    ...
    setCurrentOrder((prev) => {
      ...
      if (existingItem) {
        ...
        return prev.map((i) => (i.id === itemId ? { ...i, quantity, notes: notes || i.notes, selectedOptions: selectedOptions || i.selectedOptions } : i));
      } else {
        // Agregar nuevo item
